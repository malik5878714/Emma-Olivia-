...
const lockedCredits = Object.freeze("𝐄𝐌𝐌𝐀 𝐎𝐋𝐈𝐕𝐈𝐀");
const lockedTagline = Object.freeze("🖤 Powered by 𝐄𝐌𝐌𝐀 𝐎𝐋𝐈𝐕𝐈𝐀");

// Hook to detect if message is tampered
function verifyTagline(text) {
  if (!text.includes(lockedTagline)) {
    throw new Error("🚫 Unauthorized modification detected: 'Powered by 𝐄𝐌𝐌𝐀 𝐎𝐋𝐈𝐕𝐈𝐀' was removed or altered.");
  }
}
...
module.exports.run = async ({ api, event, args }) => {
  try {
    ...
    // ✅ Updated Render URL
    const url = `https://𝐄𝐌𝐌𝐀 𝐎𝐋𝐈𝐕𝐈𝐀-pintrest-server-wg55.onrender.com/dp?q=${encodeURIComponent(query)}&n=${count}`;
